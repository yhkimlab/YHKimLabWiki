...
- [1. 개요](#1-개요)
  - [1-1. 왜 HPC를 쓰는가: HPC vs PC](#1-1-왜-hpc를-쓰는가-hpc-vs-pc)
    - [개인 PC의 장점과 한계](#개인-pc의-장점과-한계)
    - [HPC의 장점](#hpc의-장점)
    - [ASDSLab (YHKlab) 클러스터 구성 (26.03.07)](#asdslab-yhklab-클러스터-구성-260307)
  - [1-2. 왜 Linux를 배우는가: Linux 소개](#1-2-왜-linux를-배우는가-linux-소개)
    - [Linux의 특징](#linux의-특징)
  - [1-3. 전체 구조 한눈에 보기](#1-3-전체-구조-한눈에-보기)
  - [1-4. 환경설정 순서](#1-4-환경설정-순서)
- [2. 클러스터 연결](#2-클러스터-연결)
  - [2-1. VS Code](#2-1-vs-code)
    - [VS Code의 특징](#vs-code의-특징)
  - [2-2. VS Code Remote - SSH로 접속하기](#2-2-vs-code-remote---ssh로-접속하기)
    - [(Windows) 0. 환경변수 추가](#windows-0-환경변수-추가)
    - [1. 확장 설치](#1-확장-설치)
    - [2. 원격 호스트 정보 등록](#2-원격-호스트-정보-등록)
    - [3. 서버에 접속하기](#3-서버에-접속하기)
  - [2-3. VS Code의 원격 파일 탐색기](#2-3-vs-code의-원격-파일-탐색기)
    - [원격 폴더 열기](#원격-폴더-열기)
  - [2-4. Hello Linux 예제](#2-4-hello-linux-예제)
    - [1. 로컬 PC에 `hello.sh` 준비하기](#1-로컬-pc에-hellosh-준비하기)
    - [2. 원격 작업 폴더 열기](#2-원격-작업-폴더-열기)
    - [3. 로컬 `hello.sh`를 원격 Explorer로 드래그-드롭하기](#3-로컬-hellosh를-원격-explorer로-드래그-드롭하기)
    - [4. 원격 터미널에서 위치와 파일 확인하기](#4-원격-터미널에서-위치와-파일-확인하기)
    - [5. 실행 권한 부여 후 실행하기](#5-실행-권한-부여-후-실행하기)
    - [6. VS Code에서 내용을 수정한 뒤 다시 실행하기](#6-vs-code에서-내용을-수정한-뒤-다시-실행하기)
  - [2-5. 기초 Linux 명령어](#2-5-기초-linux-명령어)
    - [예제 1: 현재 위치와 파일 확인](#예제-1-현재-위치와-파일-확인)
    - [예제 2: 실습 폴더 만들고 이동하기](#예제-2-실습-폴더-만들고-이동하기)
    - [예제 3: 파일 복사와 이름 바꾸기](#예제-3-파일-복사와-이름-바꾸기)
    - [예제 4: 파일 내용 확인과 실행](#예제-4-파일-내용-확인과-실행)
    - [예제 5: Tab 자동완성](#예제-5-tab-자동완성)
  - [2-6. X11 forwarding으로 GUI 프로그램 실행하기](#2-6-x11-forwarding으로-gui-프로그램-실행하기)
    - [X11 forwarding이 필요한 이유](#x11-forwarding이-필요한-이유)
    - [SSH 설정 예시](#ssh-설정-예시)
    - [실행 예시](#실행-예시)
- [3. 자주 생기는 문제](#3-자주-생기는-문제)
    - [VS Code가 서버에 연결되지 않는다](#vs-code가-서버에-연결되지-않는다)
    - [원격 폴더는 열렸는데 명령 실행이 이상하다](#원격-폴더는-열렸는데-명령-실행이-이상하다)
    - [드래그-드롭한 파일이 보이지 않는다](#드래그-드롭한-파일이-보이지-않는다)
    - [GUI 창이 뜨지 않는다](#gui-창이-뜨지-않는다)
- [4. 참고 자료](#4-참고-자료)
